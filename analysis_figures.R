...
PlotLabels <- function(x, srt=0, cex=1.35) {

  par(mar=rep(0,4))
  plot.new()
  plot.window(xlim=c(0,1), ylim=c(0,1))
  text(.5, .5, labels=x, srt=srt, cex=cex)
...
pdf('figures/Fig3_simulated_empirical_timeseries.pdf', 9, 4.5)
...
#---------------- Make Figure Empirical vs Model-implied --------------------------------
...
sc <- 0.6
pdf("figures/Fig4_empirical_implied_VAR.pdf", width = 10*sc, height=9*sc)

# Layout
lmat <- rbind(c(0, 1, 2),
              c(3, 5, 6),
              c(4, 7, 8))
layout(mat=lmat, widths = c(.1, 1, 1), heights = c(.1, 1, 1))

# Plot labels
PlotLabels("Lagged Effects")
PlotLabels("Residual Effects")
PlotLabels("Empirical", srt=90)
PlotLabels("Model-Implied", srt=90)
...
pdf('figures/Fig5_Wedge.pdf', 1.25*8, 1.25*5)
...
sc <- 3
nf <- layout(mat, widths = sc*c(1, 4, 1, 4), heights = sc*c(.25,4,1), TRUE)
# layout.show(nf)
...
legend(
  ...
  text.col=adjustcolor(cols, alpha = 0.60), bty = 'n',
  cex=1.5
)

# Plot marginal densities I
...
# --------------- MODEL-IMPLIED --------------------------
par(mar = c(4.25,4.25,2,2), cex.axis = 1.5, cex.lab = 1.75)
plot(
  Pos, Neg,
  pch = 20, xlim = c(0, 100), ylim = c(0, 100), axes = FALSE,
...
PlotLabels("Empirical", cex=2.2)
PlotLabels("Model-Implied", cex=2.2)
...
pdf('figures/Fig6_timescales.pdf', 9, 4)
par(mfrow = c(1, 2))
par(mar = c(3, 4, 2, 0) + 0.1)
plot(
