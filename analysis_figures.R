...
draw_rect <- function(situation, situations, max = 120) {
  ...
  rect(
    which(situations == situation), -45, which(situations == situation) + 1, -40,
...
plot_timeseries <- function(dat, main, legend = TRUE, max = 120, second_plot = FALSE) {
  ...
  if (!second_plot) {
    mtext('Variable', side = 2, line = 2.6, cex = 1.25)
...
plot_timeseries(dat_sim, 'Simulated Data', max = 120, legend = FALSE)
